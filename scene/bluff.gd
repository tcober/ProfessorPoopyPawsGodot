...
func _meet_kitty() -> void:
	...
	await theater.say("Kitty", "Here, help me put my whirligig back together")
	await theater.say("Kitty", "Gear. Spring. Crank. They're around here somewhere.")
...
func _flight_finale() -> void:
	...
	await theater.say("Kitty", "'Course it flies. I'm Kitty. Kitty Cool. And who are you?")
	await theater.say("Basil", "I'm Basil.")
...
func _scatter_parts() -> void:
	...
	# what actually flew, kept in step with `icons`. The skip below can make
	# this SHORTER than PART_KEYS, so the landing loop must walk this and not
	# the full key list — indexing icons[] by PART_KEYS would run off the end.
	var flown: Array[String] = []
	for part: String in PART_KEYS:
	...
	for i in flown.size():
		icons[i].queue_free()
		_spawn_part(flown[i], PARTS, _on_part_touched)
...
func _phase_call1() -> void:
	...
	await theater.say("Basil", "Wait - it's nearly dark, the forest track - Kitty? ...She's already pedaling.")
	theater.close_dialog()
	player.sprite.play("sit")
	await theater.wait(1.4)
	await theater.black(1.0)
...
func _phase_call2() -> void:
	...
	await theater.say("Ridley", "...Is this Basil? Your name's engraved on the little glass. I'm - there's been an accident. Out on the forest track.")
