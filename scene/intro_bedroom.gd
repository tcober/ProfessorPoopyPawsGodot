...
func _play() -> void:
	...
	await walk(basil, Vector2(320, 276), 300)   # to the doormat
	walk(basil, Vector2(320, 440), 360)         # out the door (fire-and-forget)
	await fade_out(0.7)
	finish()
...
func _show_clock() -> void:
	...
	clock_face.scale = Vector2(0.15, 0.15)
	var tw := create_tween().set_parallel()
	tw.tween_property(clock_dim, "modulate:a", 1.0, 0.25)
	tw.tween_property(clock_face, "scale", Vector2(1.25, 1.25), 0.3) \
...
func _hide_clock() -> void:
	...
	tw.tween_property(clock_face, "scale", Vector2(0.15, 0.15), 0.2)
	await tw.finished
