...
whole 768x384 scene as one composition: scene-scale grass fields (no tile can
...
p = Painter(m, "meadow", warp_scale=45.0, salt=3)
W, H = p.W, p.H
...
macro = fbm(W, H, 95.0, 3, 101, step=2)    # rolling light/dark meadow patches
hue = fbm(W, H, 150.0, 2, 106, step=4)     # warm/cool grass drift
mid = fbm(W, H, 23.0, 3, 102, step=2)      # mottle
fine = fbm(W, H, 4.5, 2, 103, step=1)      # blade-scale grain
cloud = fbm(W, H, 215.0, 2, 104, step=4)   # drifting cloud shade
wfld = fbm(W, H, 75.0, 2, 105, step=4)     # trail width variation

# organic boundaries: coarse+blurred SDFs melt the 16px tile steps entirely
sdf_tree = p.sdf("#", blur=8, step=4)
sdf_pond = p.sdf("o", blur=10, step=4)

# the trail: a Catmull-Rom spline tracking the map's '-' cells (tile coords x16)
...
def s_path_at(x, y):
    ...
    hw = 6.0 + (wfld.sample(x, y) - 0.5) * 3.0
    dx = x - TRAIL_END[0]
    dy = y - TRAIL_END[1]
    d2 = dx * dx + dy * dy
    if d2 < 400.0:                       # trailhead widens toward the cairn
        hw += (20.0 - d2 ** 0.5) * 0.24
...
for y in range(H):
    ...
    for x in range(W):
        ...
        if s_pond <= 0.0:
        ...
        elif s_pond <= 3.0 and mid_(x, y) > 0.36:
            # wet-sand collar (dry-path tones), broken where grass meets water
            t = 0.24 + s_pond * 0.08 + (f - 0.5) * 0.16 + light
            c = pb[tone_i(5, t, x, y, 22)]
        else:
            hw = 6.0 + (wf_(x, y) - 0.5) * 3.0
            dx = x - EX
            dy = y - EY
            d2 = dx * dx + dy * dy
            if d2 < 400.0:
            ...
            md = mid_(x, y)
            if s_path <= 0.0 and not (s_path > -1.5 and md > 0.72):
                ...
                s_tree = st_(wxp, wyp)
                if s_tree < 22.0:
                    t += (22.0 - s_tree) * 0.009    # treeline ambient occlusion
                if 0.0 < s_path <= 3.0:
                    t += 0.10                       # trodden fringe
                # warm drift patches, noise-dithered edge, kept off the treeline AO
                warm = s_tree > 15.0 and hue_(x, y) + (f - 0.5) * 0.2 > 0.62
                cl = clo_(x, y)
                if cl > 0.58:
...
for (tx, ty) in rocks:
    cx = tx * ZONE_TILE + 8 + (h2(tx, ty, 51) % 5) - 2
    cy = ty * ZONE_TILE + 8 + (h2(tx, ty, 52) % 3) - 1
    stamp_boulder(p, cx, cy, 9 + h2(tx, ty, 53) % 2, ROCK, GRASS, SHADOW, salt=tx * 7 + ty)

# trailhead cairn — the focal point the path leads to (walkable; just paint)
cx, cy = int(TRAIL_END[0]), int(TRAIL_END[1])
stamp_boulder(p, cx, cy - 2, 7, ROCK, GRASS, SHADOW, salt=201)
stamp_boulder(p, cx - 1, cy - 7, 4, ROCK, GRASS, SHADOW, salt=202)
for k in range(7):
...
for (x, y) in p.scatter(".f-", cell=11, keep=0.55, salt=61, inset=2):
    if s_path_at(x, y) > 1.0 and sdf_pond.sample(x, y) > 6.0:
...
for ty in range(m.rows_n):
    for tx in range(m.cols):
        ...
        for k in range(2 + h2(tx, ty, 71) % 2):
            fx = tx * ZONE_TILE + 2 + h2(tx + k, ty, 72) % 12
            fy = ty * ZONE_TILE + 2 + h2(tx, ty + k, 73) % 12
            if s_path_at(fx, fy) > 2.0:
        ...
        for k in range(3):  # baby's-breath specks between the clumps
            ...
            if s_path_at(sx, sy) > 1.5:
                p.ground.put(sx, sy, (246, 244, 236, 255))

for (x, y) in p.scatter(".", cell=48, keep=0.30, salt=81, inset=4):
    if s_path_at(x, y) > 5.0 and sdf_pond.sample(x, y) > 8.0 and sdf_tree.sample(x, y) > 10.0:
...
for (tx, ty) in p.tiles("-", pad=1):
    ...
    s = s_path_at(x, y)
    if 1.0 <= s <= 4.5 and sdf_pond.sample(x, y) > 6.0:
        stamp_pebbles(p, x, y, ROCK, salt=94)

SPARKLE = (214, 246, 248, 255)
for (x, y) in p.scatter("o", cell=22, keep=0.45, salt=95, inset=6):
