...
overworld_overlay.png (1024x576 = 64x36 tiles from assets/maps/overworld.txt).
...
p = Painter(m, "overworld", warp_scale=55.0, salt=7)
W, H = p.W, p.H
...
macro = fbm(W, H, 120.0, 3, 201, step=2)
mid = fbm(W, H, 26.0, 3, 202, step=2)
fine = fbm(W, H, 5.0, 2, 203, step=1)
cloud = fbm(W, H, 260.0, 2, 204, step=4)
crack = fbm(W, H, 17.0, 2, 205, step=2)    # dedicated field — never sample a
                                           # Field outside its domain (it clamps)

sdf_sea = p.sdf("~", blur=10, step=4)
sdf_forest = p.sdf("f", blur=8, step=4)
sdf_mtn = p.sdf("M", blur=6, step=4)
sdf_river = p.sdf("r", blur=6, step=2)   # narrow ribbon: fine grid, strong smooth
sdf_bridge = p.sdf("=", blur=1, step=2)
sdf_waste = p.sdf("x", blur=8, step=4)
sdf_hills = p.sdf("h", blur=6, step=4)
...
for y in range(H):
    ...
    for x in range(W):
        ...
        if ssea <= 0.0:
            ...
            if depth <= 1.2:
                c = sb_[5]                                     # waterline
            elif depth <= 2.8 and f > 0.46:
                c = sb_[0]                                     # inner foam arc
            elif 4.5 <= depth <= 6.3 and f > 0.60:
                c = sb_[1]                                     # outer broken arc
            else:
                d = depth / 75.0                               # gentle: enclosed
            ...
            sbrg = brg_(wxp, wyp)
            if sbrg <= 1.5:                                    # rosewood planks
                t = 0.30 + ((y // 2) % 2) * 0.14 + (f - 0.5) * 0.12 + light
                if sbrg > 0.5:
                ...
                if sriv <= 1.5:
                    if sriv > 0.7:
                        ...
                        t = 0.30 - (0.10 if sriv < -1.5 else 0.0)
...
for (x, y) in p.scatter("x", cell=32, keep=0.45, salt=51, inset=5):
    if h2(x, y, 52) < 130:                                     # dead tree
        p.cast_shadow(x + 2, y + 1, 5, 2, SHADOW, 0.3)
        for i in range(6):
            ground.put(x, y - i, DEAD[1 if i < 4 else 0])
            ground.put(x + 1, y - i, DEAD[2])
        for (bx, by) in ((-2, -4), (2, -5), (-1, -6), (2, -3)):
        ...
        ground.put(x, y - 5, (244, 226, 255, 255))
        for r_ in range(2, 4):
            ground.mix(x - r_, y - 2, ACCENT, 0.10)
            ground.mix(x + r_, y - 2, ACCENT, 0.10)

# sea sparkles
for (x, y) in p.scatter("~", cell=45, keep=0.30, salt=61, inset=12):
...
for name in ("home", "meadow", "town", "cave", "obelisk"):
    ...
    cx, cy = tx * ZONE_TILE + 8, ty * ZONE_TILE + 8
